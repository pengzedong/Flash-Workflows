...
# for i in $(find {Paramesh4.0,Paramesh4dev}/PM4_package/{headers,source,utilities/multigrid} \
...
def depends(filename):
    ...
    for x in io.open(filename,"r",encoding="utf-8").readlines():
        m = modRE.match(x)
        if m:
           if not intrsmodRE.match(m.group(1)) : mods[m.group(1)] = 1
           continue
        m = incRE.match(x)
        if m: 
           incs[m.group(1)] = 1
...
##################
def main():
   global modRE, intrsmodRE, incRE, defmodRE, reordRE, parameshFileRE
   ...
   intrsmodRE = re.compile(intrinsicModnamepatt,re.I)
