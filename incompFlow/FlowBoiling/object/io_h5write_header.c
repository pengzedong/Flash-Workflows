...
#ifdef FLASH_IO_ASYNC_HDF5
  dataset = H5Dcreate_async(*file_identifier, "sim info", si_type,
...
#ifdef FLASH_IO_ASYNC_HDF5
    status = H5Dwrite_async(dataset, si_type, H5S_ALL, H5S_ALL,
                  H5P_DEFAULT, &sim_info, io_es_id);
#else
  ...
  if(*nvar_out > 0){
...
#ifdef FLASH_IO_ASYNC_HDF5
    dataset   = H5Dcreate_async(*file_identifier, "unknown names", 
    ...
    if ((*MyPE == 0) || (*io_outputSplitNum != 1)) {
      ...
      status    = H5Dwrite_async(dataset, string_type, H5S_ALL, H5S_ALL, 
                     H5P_DEFAULT, unk_labels, io_es_id);
#else
